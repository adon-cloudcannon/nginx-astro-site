...
<Code_Block language="yaml" hints={[{ number: "1", display_text: "203.0.113.1", tooltip: "Arbitrary host address", nxt_type: "nxt_hint" }]}>{`all:
  children:
    unit_hosts:
      hosts:
        NXT1:`}</Code_Block>
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "inventory.yaml", tooltip: "Inventory filename", nxt_type: "nxt_hint" }, { number: "2", display_text: "playbook.yaml", tooltip: "Playbook filename", nxt_type: "nxt_hint" }]}>{`$ ansible-playbook -i NXT1 NXT2
      ...
      PLAY RECAP ********************************
      203.0.113.1                  : ok=15   changed=0    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0`}</Code_Block>
