...
<Test_Component language="bash" hints={[{ number: "1", display_text: "display", tooltip: "tooltip" }]}>{`$ docker exec -ti $UNIT curl -X PUT --data-binary @/www/config.json  \\
      --unix-socket NXT1  \\
      http://localhost/config`}</Test_Component>
