...
<Code_Block language="nginx" hints={[{ number: "1", display_text: "127.0.0.1:8080", tooltip: "Unit's control socket address", nxt_type: "nxt_ph" }]}>{`location / {
    proxy_pass http://NXT1;
}`}</Code_Block>