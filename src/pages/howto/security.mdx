...
<Code_Block language="bash" hints={[{ number: "1", display_text: "drwxr-xr-x", tooltip: "Permissions are OK", nxt_type: "nxt_hint" }, { number: "2", display_text: "drwxr-x---", tooltip: "Permissions are too restrictive", nxt_type: "nxt_hint" }]}>{`# ls -l /        NXT1  some_user some_group  path  # ls -l /path/        NXT2  some_user some_group  to `}</Code_Block>
...
`# :nxt_hint:chown &lt;Assign ownership for the app code&gt; -R app_user:app_group :nxt_ph:/path/to/app/ &lt;Path to the application directory; use a real path in your command&gt; # :nxt_hint:chown &lt;Assign ownership for the static files&gt; -R app_user:app_group :nxt_ph:/path/to/static/app/files/ &lt;Can be outside the app directory tree; use a real path in your command&gt; # find :nxt_ph:/path/to/app/ &lt;Path to the application directory; use a real path in your command&gt; -type d -exec :nxt_hint:chmod &lt;Add read/execute permissions to app code directories for user and group&gt; u=rx,g=rx,o= {} ; # find :nxt_ph:/path/to/static/app/files/ &lt;Can be outside the app directory tree; use a real path in your command&gt; -type d -exec :nxt_hint:chmod &lt;Add read/execute permissions to static file directories for user and group&gt; u=rx,g=rx,o= {} ; `
