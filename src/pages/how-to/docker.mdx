...
<Code_Block language="bash" hints={[{ number: "1", display_text: "/var/run/control.unit.sock", tooltip: "Socket path inside the container" }]}>{`$ docker exec -ti $UNIT curl -X PUT --data-binary @/www/config.json  \\
      --unix-socket NXT1  \\
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "/www/", tooltip: "Directory inside the container where the app files will be stored" }, { number: "2", display_text: "wsgi", tooltip: "WSGI module basename with extension omitted" }, { number: "3", display_text: "app", tooltip: "Name of the callable in the module to run" }]}>{`$ mkdir config
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "unit-webapp", tooltip: "Arbitrary image tag" }]}>{`$ docker build --tag=NXT1 .`}</Code_Block>
