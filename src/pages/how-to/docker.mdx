...
<Code_Block language="bash" hints={[]}>{`$ cd :nxt_ph:`/path/to/app/ <Directory where all app-related files will be stored; use a real path in your configuration>`
$ mkdir webapp
...
<Code_Block language="bash" hints={[]}>{`$ cat << EOF > requirements.txt
