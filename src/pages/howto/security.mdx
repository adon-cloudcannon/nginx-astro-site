...
* If you install Unit from [source files](https://unit.nginx.org/installation/#source), rebuild and reinstall Unit and its modules from scratch.
...
<Dropdown label="State Directory">
...
<Dropdown label="File Permissions">
...
<Code_Block language="bash" hints={[]}>{`# usermod -a -G app_group unit_user`}</Code_Block>
...
<Code_Block language="bash" hints={[]}>{`# usermod -a -G some_group app_user`}</Code_Block>
...
<Dropdown label="Unit's Process Summary">
...
<Code_Block language="bash" hints={[]}>{`$ ps aux | grep unit

      ...
      root   ... unit: main v1.32.1
      unit   ... unit: controller
      unit   ... unit: router
      unit   ... unit: "front" application`}</Code_Block>
