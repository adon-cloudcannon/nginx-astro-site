...
<Code_Block language="bash" hints={[{ number: "1", display_text: "/var/run/control.unit.sock", tooltip: "Socket path inside the container", nxt_type: "nxt_hint" }]}>{`$ docker exec -ti $UNIT curl -X PUT --data-binary @/www/config.json  \\
      --unix-socket NXT1  \\
      http://localhost/config`}</Code_Block>
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "/path/to/app", tooltip: "Directory where all app-related files will be stored; use a real path in your configuration", nxt_type: "nxt_ph" }]}>{`$ cd NXT1
$ mkdir webapp
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "/www/", tooltip: "Directory inside the container where the app files will be stored", nxt_type: "nxt_hint" }, { number: "2", display_text: "wsgi", tooltip: "WSGI module basename with extension omitted", nxt_type: "nxt_hint" }, { number: "3", display_text: "app", tooltip: "Name of the callable in the module to run", nxt_type: "nxt_hint" }]}>{`$ mkdir config
...
<Code_Block language="none" hints={[{ number: "1", display_text: "/path/to/app", tooltip: "Directory where all app-related files are stored; use a real path in your configuration", nxt_type: "nxt_ph" }]}>{`NXT1
├── config
│   └── config.json
├── log
│   └── unit.log
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "unit-webapp", tooltip: "Arbitrary image tag", nxt_type: "nxt_hint" }]}>{`$ docker build --tag=NXT1 .`}</Code_Block>
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "/path/to/app/", tooltip: "Directory where all app-related files are stored", nxt_type: "nxt_ph" }, { number: "2", display_text: "/new/path/to/app/", tooltip: "New directory; use a real path in your configuration", nxt_type: "nxt_ph" }]}>{`$ mv NXT1 NXT2`}</Code_Block>
...
<Code_Block language="json" hints={[{ number: "1", display_text: "/www/", tooltip: "Directory inside the container where the app files will be stored", nxt_type: "nxt_hint" }, { number: "2", display_text: "/usr/bin/env", tooltip: "The external app type allows to run arbitrary executables, provided they establish communication with Unit", nxt_type: "nxt_hint" }, { number: "3", display_text: "arguments", tooltip: "The env executable runs Node.js, supplying Unit's loader module and your app code as arguments", nxt_type: "nxt_hint" }, { number: "4", display_text: "app.js", tooltip: "Basename of the application file; be sure to make it executable", nxt_type: "nxt_ph" }]}>{`{
    "listeners": {
        "*:8080": {
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "unit-expressapp", tooltip: "Arbitrary image tag", nxt_type: "nxt_hint" }]}>{`$ docker build --tag= .
...
<Code_Block language="docker" hints={[{ number: "1", display_text: "unit-jsc11 unit-perl unit-php unit-python2.7 unit-python3.9 unit-ruby", tooltip: "Leave only packages for the language you need, removing the rest", nxt_type: "nxt_hint" }]}>{`FROM unit:1.32.1-minimal
...
RUN apt update && apt install -y                                              \\
    ...
    && apt autoremove --purge -y                                              \\
    && rm -rf /var/lib/apt/lists/* /etc/apt/sources.list.d/*.list`}</Code_Block>
