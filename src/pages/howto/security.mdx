...
<Dropdown label="Details">{`Specific upgrade steps depend on your installation method:

* The recommended option is to use our official [packages](https://unit.nginx.org/installation/#installation-precomp-pkgs) or Docker [images](https://unit.nginx.org/installation/#installation-docker); with them, it’s just a matter of updating **unit-\*** packages with your package manager of choice or switching to a newer image.
* If you use a third-party installation [method](https://unit.nginx.org/installation/#installation-community-repos), consult the maintainer’s documentation for details.
* If you install Unit from [source files](https://unit.nginx.org/installation/#source), rebuild and reinstall Unit and its modules from scratch.`}</Dropdown>
...
<Dropdown label="Control Socket">{`If you use a UNIX control socket, ensure it is available to **root** only:
...
<Code_Block language="bash" hints={[]}>{`# unitd --control 203.0.113.14:8080
...
<Code_Block language="bash" hints={[]}>{`# unitd --control 127.0.0.1:8080
$ curl 203.0.113.14:8080

    curl: (7) Failed to connect to 203.0.113.14 port 8080: Connection refused`}</Code_Block>

However, any processes local to the same system can access the local socket, which calls for additional measures. A go-to solution would be using NGINX to [proxy](integration.md#nginx-secure-api) Unit’s control API.`}</Dropdown>
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "chmod", tooltip: "Add write permissions for the user only; the group shouldn't have them", nxt_type: "nxt_hint" }, { number: "2", display_text: "/path/to/writable/file/or/directory/", tooltip: "Repeat for each file or directory that must be writable", nxt_type: "nxt_ph" }]}>{`# NXT1 u+w NXT2`}</Code_Block>

In case of a writable directory, you may also want to prevent non-owners from messing with its files:

<Code_Block language="bash" hints={[{ number: "1", display_text: "chmod", tooltip: "Sticky bit prevents non-owners from deleting or renaming files", nxt_type: "nxt_hint" }, { number: "2", display_text: "/path/to/writable/directory/", tooltip: "Repeat for each directory that must be writable", nxt_type: "nxt_ph" }]}>{`# NXT1 +t NXT2`}</Code_Block>
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "/path/to/app/code/", tooltip: "Path to the application's code directory; use a real path in your command", nxt_type: "nxt_ph" }, { number: "2", display_text: "chmod", tooltip: "Add read rights to app code for user and group", nxt_type: "nxt_hint" }, { number: "3", display_text: "/path/to/static/app/files/", tooltip: "Can be outside the app directory tree; use a real path in your command", nxt_type: "nxt_ph" }, { number: "4", display_text: "chmod", tooltip: "Add read rights to static files for user and group", nxt_type: "nxt_hint" }]}>{`# find NXT1 -type f -exec NXT2 u=r,g=r,o= {} ;
# find NXT3 -type f -exec NXT4 u=r,g=r,o= {} ; `}</Code_Block>

7\. For [external](modules.md#modules-emb) apps, additionally make the app code or binaries executable:

<Code_Block language="bash" hints={[{ number: "1", display_text: "/path/to/app/", tooltip: "Path to the application directory; use a real path in your command", nxt_type: "nxt_ph" }, { number: "2", display_text: "chmod", tooltip: "Add read and execute rights to app code for user and group", nxt_type: "nxt_hint" }, { number: "3", display_text: "/path/to/static/app/files/", tooltip: "Can be outside the app directory tree; use a real path in your command", nxt_type: "nxt_ph" }, { number: "4", display_text: "chmod", tooltip: "Add read rights to static files for user and group", nxt_type: "nxt_hint" }]}>{`# find NXT1 -type f -exec NXT2 u=rx,g=rx,o= {} ;
# find NXT3 -type f -exec NXT4 u=r,g=r,o= {} ; `}</Code_Block>

8\. To run a single app, [configure](../configuration.md) Unit as follows:

<Code_Block language="json" hints={[{ number: "1", display_text: "*:80", tooltip: "Or another suitable socket address", nxt_type: "nxt_hint" }, { number: "2", display_text: "/path/to/static/app/files/", tooltip: "Router process needs read and execute permissions to serve static content from this directory", nxt_type: "nxt_ph" }]}>{`{
    "listeners": {
        "NXT1": {
            "pass": "routes"
        }
...
<Code_Block language="json" hints={[{ number: "1", display_text: "*:80", tooltip: "Or another suitable socket address", nxt_type: "nxt_hint" }, { number: "2", display_text: "/app1/*", tooltip: "Arbitrary matching condition", nxt_type: "nxt_hint" }, { number: "3", display_text: "/path/to/static/app1/files/", tooltip: "Router process needs read and execute permissions to serve static content from this directory", nxt_type: "nxt_ph" }, { number: "4", display_text: "/app2/*", tooltip: "Arbitrary matching condition", nxt_type: "nxt_hint" }, { number: "5", display_text: "/path/to/static/app2/files/", tooltip: "Router process needs read and execute permissions to serve static content from this directory", nxt_type: "nxt_ph" }]}>{`{
    "listeners": {
        "NXT1": {
            "pass": "routes"
        }
...
<Code_Block language="json" hints={[{ number: "1", display_text: "uri", tooltip: "Handles requests that target PHP scripts to avoid having them served as static files", nxt_type: "nxt_hint" }, { number: "2", display_text: "uri", tooltip: "Protects files and directories best kept hidden", nxt_type: "nxt_hint" }, { number: "3", display_text: "!/sensitive/*", tooltip: "Restricts access to a directory with sensitive data", nxt_type: "nxt_hint" }, { number: "4", display_text: "!/data/*", tooltip: "Restricts access to a directory with sensitive data", nxt_type: "nxt_hint" }, { number: "5", display_text: "!/app_config_values.ini", tooltip: "Restricts access to a specific file", nxt_type: "nxt_hint" }, { number: "6", display_text: "!*/.*", tooltip: "Restricts access to hidden files and directories", nxt_type: "nxt_hint" }, { number: "7", display_text: "!*~", tooltip: "Restricts access to temporary files", nxt_type: "nxt_hint" }, { number: "8", display_text: "share", tooltip: "Serves valid requests with static content", nxt_type: "nxt_hint" }, { number: "9", display_text: "/path/to/app/static", tooltip: "Path to the application's static file directory; use a real path in your configuration", nxt_type: "nxt_ph" }, { number: "10", display_text: "types", tooltip: "Limits file types served from the share", nxt_type: "nxt_hint" }, { number: "11", display_text: "fallback", tooltip: "Relays all requests not yet served to a catch-all app target", nxt_type: "nxt_hint" }]}>{`{
    "routes": {
        "app": [
            {
                "match": {
...
<Code_Block language="none" hints={[{ number: "1", display_text: "/path/to/unit.log", tooltip: "Use a real path in your configuration", nxt_type: "nxt_ph" }, { number: "2", display_text: "/path/to/unit.pid", tooltip: "Use a real path in your configuration", nxt_type: "nxt_ph" }, { number: "3", display_text: "/bin/kill", tooltip: "Signals Unit to reopen the log", nxt_type: "nxt_hint" }, { number: "4", display_text: "/path/to/unit.pid", tooltip: "Use a real path in your configuration", nxt_type: "nxt_ph" }]}>{`NXT1 {
    daily
    missingok
    rotate 7
    compress
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "/default/path/to/unit.pid", tooltip: "Build-time setting, can be overridden", nxt_type: "nxt_ph" }, { number: "2", display_text: "/default/path/to/unit.log", tooltip: "Build-time setting, can be overridden", nxt_type: "nxt_ph" }, { number: "3", display_text: "/path/to/unit.pid", tooltip: "Make sure to check for runtime overrides", nxt_type: "nxt_ph" }, { number: "4", display_text: "/path/to/unit.log", tooltip: "Make sure to check for runtime overrides", nxt_type: "nxt_ph" }]}>{`$ unitd -h
...
$ ps ax | grep unitd
      ...
      ... unit: main v1.32.1 [... --pid NXT3 --log NXT4 ...]`}</Code_Block>
...
<Code_Block language="bash" hints={[{ number: "1", display_text: "chown", tooltip: "Assign ownership to the log user", nxt_type: "nxt_hint" }, { number: "2", display_text: "/path/to/unit.log", tooltip: "If it's overridden, use the runtime setting", nxt_type: "nxt_ph" }, { number: "3", display_text: "curl", tooltip: "Enable access log in the Unit configuration at the given path", nxt_type: "nxt_hint" }, { number: "4", display_text: "/path/to/access.log", tooltip: "Use a real path in your configuration", nxt_type: "nxt_ph" }, { number: "5", display_text: "/path/to/control.unit.sock", tooltip: "Path to Unit's control socket", nxt_type: "nxt_ph" }, { number: "6", display_text: "chown", tooltip: "Assign ownership to the log user", nxt_type: "nxt_hint" }, { number: "7", display_text: "/path/to/access.log", tooltip: "Use a real path in your command", nxt_type: "nxt_ph" }]}>{`# NXT1 log_user:log_group NXT2
...
# NXT6 log_user:log_group NXT7`}</Code_Block>

If you change the log file ownership, adjust your **logrotate** settings accordingly:

<Code_Block language="none" hints={[{ number: "1", display_text: "/path/to/unit.log", tooltip: "Use a real path in your configuration", nxt_type: "nxt_ph" }]}>{`NXT1 {
    ...
